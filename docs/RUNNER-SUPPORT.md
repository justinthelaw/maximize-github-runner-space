...
an action. The runtime rejects unrecognized runner-image records, but a larger
...
Every VM requires the fixed image-data record written by its runner-images
definition: `/imagegeneration/imagedata.json` on Ubuntu,
`/Users/runner/imagedata.json` on macOS, or `C:\imagedata.json` on Windows. The
runtime accepts exactly one supported `Runner Image` record and cross-checks its
label, numeric version, source branch, manifest path, release link, operating
system, and architecture. `ImageOS`, `ImageVersion`, `HOME`, and other ordinary
workflow-overridable values cannot select an image identity. Missing,
malformed, linked, oversized, duplicated, or mismatched records fail before an
adapter or package manager is initialized.

Container cleanup requires the distinct runner-images
...
fail before mutation; a full-VM record cannot authorize container cleanup.

These local files prove compatibility with a runner-images definition; they
are not signed attestation. GitHub-hosted jobs have root or administrator
access, so a prior privileged step can deliberately replace local image state.
The gate prevents environment-variable spoofing and accidental execution on an
unrecognized layout, but it does not establish a trust boundary against earlier
malicious workflow code or distinguish a larger runner retaining the identical
official image.
...
| Runner-image compatibility | Fixed, bounded image-data record generated by runner-images; ordinary `ImageOS` / `ImageVersion` values are ignored for authorization |
...
| Rust | Definition-owned `C:\Users\Default\.cargo` and `.rustup` seed roots, plus matching roots in the runtime user's home ([installer](https://github.com/actions/runner-images/blob/20f9f7b2d2dbcf53e5c5a7e133f4867e8a555c24/images/windows/scripts/build/Install-Rust.ps1)) | Workflow path overrides are ignored |
