...
Docker Engine removal also removes Docker's runner data. To keep any existing
images, containers, volumes, or build cache with `max`, skip both
`docker-engine` and `docker-images`.
...
stages exact `fstab` content, and atomically exchanges it with the live file.
It then verifies the content, file identity, mode, and owner of both files. A
concurrent writer is preserved and stops the transaction. If recovery cannot
be proven, the action keeps every active swap and backing file needed by a
possible live `fstab` entry. Staging also requires protected `/mnt` and `/etc`
parents and rechecks each private mode-`0600` temporary file before mutation.
...
| `remove-docker-engine`  | `docker-engine`  | Linux, Windows        | Stops and removes Docker Engine packages, helpers, data, and its exact Windows service registration where installed.        |
...
| `remove-docker-images`  | `docker-images`  | Linux, Windows        | Removes unused Docker containers, images, build cache, networks, and anonymous volumes. Named volumes are retained.         |
...
Every cleanup failure is terminal. Important examples include:
...
- Windows installer exit `1641` (restart initiated); and
- unsafe macOS Homebrew or Docker configuration isolation.

Windows installer exit `3010` (restart required) counts as completed only when
the removal postcondition passes.

Every cleanup failure stops later operations and fails the step. A failure
during preflight rolls back earlier reversible service stops. After package,
filesystem, or system cleanup starts, services stay stopped so the action does
not restart a partly removed installation. State-neutral configuration cleanup
can still run. If a timed-out process cannot be confirmed dead, no rollback
command starts because it could race that process. Successful operations report
`removed`, `not-found`, or `unsupported`.

Destructive commands use fixed runner-image executable paths, restricted
environments, bounded inventories, and postconditions. Docker and macOS
Homebrew use isolated configuration. This model assumes a fresh standard
hosted runner; it is not a sandbox or signed attestation. Unix recursive
deletion is anchored to validated directory handles. It blocks device changes
on Linux and macOS, plus same-device bind mounts on Linux. Windows recursive
deletion locks the validated path and deletes entries by native handle. Other
package and installer transactions can still be invalidated by earlier or
concurrent changes. Run cleanup before checkout when possible. If checkout
must run first, pin it, disable credential persistence, and do not execute
repository code before cleanup. See the full
[safety boundary](RUNNER-SUPPORT.md#safety-and-result-semantics).
